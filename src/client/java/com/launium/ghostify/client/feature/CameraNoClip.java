...
public class CameraNoClip extends AbstractModule implements ClientTickEvents.StartTick {
    ...
    private static final KeyMapping CAMERA_NO_CLIP_KEY = KeyBindingHelper.registerKeyBinding(
            new KeyMapping("key.ghostify.switch_camera_no_clip", InputConstants.Type.KEYSYM, GLFW.GLFW_KEY_F7, KeyMapping.CATEGORY_MISC)
    );
    public boolean isEnabled = false;

    @Override
    public void onStartTick(Minecraft client) {
        if (CAMERA_NO_CLIP_KEY.consumeClick()) {
            isEnabled = !isEnabled;
